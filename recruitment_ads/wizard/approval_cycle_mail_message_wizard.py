...
class ApprovalCycleMailComposeMessage(models.TransientModel):
    _name = 'approval.cycle.mail.compose.message'
    _inherit = 'mail.compose.message'
    _description = 'Approval cycle Mail wizard'

    approval_user = fields.Many2one('res.partner', string='to')
    recruiter_id = fields.Many2many('res.partner', 'approval_mail_compose_message_res_partner_rel', 'wizard_id',
  ...
  border: 1px solid black;
    ...
    def send_mail(self, auto_commit=False):
        ...
        for wizard in self:
            ...
            object = self.env['hr.approval.cycle.users'].search([('id','=',wizard.res_id)])
            if object:
                object.sent = True
                object.approval_cycle_id.state = 'pending'
                object.email_id = batch_mails.id
