odoo.define('recruitment_ads.animation', function (require) {
'use strict';
    ...
    sAnimation.registry.form_builder_send = sAnimation.Class.extend({
        ...
        update_status: function (status) {
            ...
            var error_html='<span id="o_website_form_result" class="text-danger ml8"><i class="fa fa-close mr4"/>Applied before on the selected Job.</span>'
            if (status == 'error' && model_name){
                if (model_name == 'hr.applicant'){
                    this.$target.find('#o_website_form_result').replaceWith(error_html);
                    return
                }
            }
