...
class Applicant(models.Model):
    ...
    def _select(self):
        ...
        select_str = """
             ...
             color ,emp_id,response_id,campaign_id ,medium_id , message_last_post ,activity_date_deadline
              ,last_activity, last_activity_date, result , face_book ,linkedin from hr_applicant
        """
        return select_str
    ...
    def column_exists(self):
        ...
        query = """ SELECT column_name FROM information_schema.columns
                            WHERE table_name=%s AND  (column_name = %s or column_name = %s or column_name = %s) """
        result=self.env.cr.execute(query, ('hr_applicant', 'last_activity','last_activity_date','result'))
        # return result
